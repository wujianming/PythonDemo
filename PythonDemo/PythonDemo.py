...
def crawerEach(url,urldir):
    ...
    for item in items:
        ...
        if target.u ==  None and target.b == None and target.font == None:
            urldir[target.text] = "http://pw.lovecl.pw/"+target.get('href')
    return urldir
#爬下所有文章的标题的URL地址    
def crawer():
    urldir={}
    for i in range(39):
        url="http://pw.lovecl.pw/thread0806.php?fid=20&page="+str(i+1)
...
def getContent(soup , author ,url, pageAccount):
    ...
    i = 2
    while i<=pageInt:
        pageUrl = "http://pw.lovecl.pw/read.php?tid=" + tid + "&page=" + str(i)
        print(pageUrl)
...
#获得图片
def getPicture(url):
    #url="http://pw.lovecl.pw/htm_data/8/1412/1313643.html"
    headers = {'User-Agent':'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.9.0.7) Gecko/2009021910 Firefox/3.0.7'}
